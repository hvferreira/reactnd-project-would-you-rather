...
class QuestionRespView extends Component {
    state = {
        selectedValue: null
    }
    handleChange = (e) => {
    ...
    onSubmit = (e) => {
        e.preventDefault()
        this.props.dispatch(handleSaveAnswer(this.props.authedUser, this.props.match.params.question_id, this.state.selectedValue))
    }
    render() {
        if (this.props.error) {
            return (
...
function mapStateToProps({ users, questions, authedUser }, { match }) {
    ...
    let q = questions[match.params.question_id]
    let author = q ? users[q.author] : ''
    return {
        q: questions[match.params.question_id],
        author,
        authedUser
    }
}

export default connect(mapStateToProps)(QuestionRespView)